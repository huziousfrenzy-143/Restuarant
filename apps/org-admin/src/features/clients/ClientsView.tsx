...
export const ClientsView: React.FC<ClientsViewProps> = ({
  ...
  const handleOpenWhatsApp = (cli: Client) => {
    const rawPhone = (cli.phone || '').replace(/[^0-9]/g, '');
    const dueAmount = Number(cli.credit_balance) || 0;

    let message = `Hello ${cli.name}, this is a polite account statement reminder from your store. `;
    if (dueAmount > 0) {
    ...
    const targetUrl = `https://wa.me/${rawPhone}?text=${encodedMsg}`;
                      ...
                      <div className="relative inline-block text-left">
                        ...
                        {isMenuOpen && (
                          <div className="absolute right-0 mt-1 w-56 rounded-md bg-surface border border-mist shadow-lg z-30 py-1 font-mono text-xs text-left animate-in fade-in-50 zoom-in-95">
                            ...
                            <button
                              onClick={() => { setActiveMenuId(null); handleOpenWhatsApp(cli); }}
                              className="w-full px-3 py-2 text-left hover:bg-emerald-50 text-emerald-800 flex items-center gap-2 transition-colors font-semibold border-t border-mist/60"
                            >
                              <MessageSquare className="w-3.5 h-3.5 text-emerald-600 shrink-0" />
                              <span>WhatsApp Chat Reminder</span>
                            </button>
                          </div>
                        )}
                      </div>
