...
import { ShieldCheck, Mail, Lock, KeyRound, ArrowRight, CheckCircle2, Sparkles, AlertCircle } from 'lucide-react';
import { API_BASE_URL } from '../config/api';

interface SuperAdminLoginViewProps {
  onLoginSuccess: (accessToken: string, refreshToken: string, email: string) => void;
}
...
export const SuperAdminLoginView: React.FC<SuperAdminLoginViewProps> = ({ onLoginSuccess }) => {
  ...
  const [email, setEmail] = useState('arhamsaifofficial@gmail.com');
  ...
  // Step 1: Request OTP via dedicated Super Admin Auth route
  const handleRequestOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccessMsg(`OTP sent to ${email}! Please check your email inbox.`);
      setStep('verify_otp');
    } catch (err: any) {
  ...
  // Step 2: Verify 6-digit OTP via dedicated Super Admin Auth route
  const handleVerifyOtp = async (e: React.FormEvent) => {
    e.preventDefault();
    if (otp.length !== 6) {
      setErrorMsg('Please enter the full 6-digit verification OTP');
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-steel flex items-center justify-center p-6 selection:bg-primary/20">
      <div className="w-full max-w-md bg-surface border border-mist rounded-xl shadow-xl overflow-hidden animate-in fade-in zoom-in duration-200">
        {/* Header */}
        <div className="p-6 bg-[#1B1D1F] text-white text-center space-y-2">
          <div className="w-12 h-12 rounded-lg bg-primary text-white font-mono font-bold text-xl flex items-center justify-center mx-auto shadow-lg">
            SA
          </div>
          <h1 className="text-xl font-bold tracking-tight">Super Admin Security Access</h1>
          <p className="text-xs text-gray-400 font-mono">Platform Operator Control Plane · Email OTP Verification</p>
        </div>

        {/* Step 1: Request OTP */}
        {step === 'request_otp' && (
          <form onSubmit={handleRequestOtp} className="p-6 space-y-4 font-sans">
            {errorMsg && (
            ...
            <div>
              <label className="text-xs font-mono text-graphite block mb-1">Super Admin Email</label>
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-9 pr-3 py-2.5 rounded-md border border-mist text-xs focus:outline-none focus:ring-1 focus:ring-primary bg-surface font-mono font-bold"
                  placeholder="arhamsaifofficial@gmail.com"
                />
              </div>
            </div>

            <div>
              <label className="text-xs font-mono text-graphite block mb-1">Operator Password</label>
              <div className="relative">
            ...
            >
              {isLoading ? 'Sending Security OTP...' : 'Send Login Verification OTP'}
